...
def reportIssue(filename, line, rule, description):
    report(10, "tin", "tin", filename, line, "opencanary", rule, "oc_cpp_issues", description, "[3](https://gitlab.kindtechnologies.nl/OOAKT/tin/blob/master/" + stripPreFix(filename) + "#L" + str(line) + ")")

def report(priority, team, component, filename, line, source, rule, category, description, link):
    s = "|"
    sprint(str(priority) + s + team + s + component + s + makeRelative(filename) + ":" + str(line) + s + source + s + rule + s + category + s + description + s + link)
